...
public class SearchGUI extends JFrame {
   ...
   /* ----------------------------------------------- */
    ...
    void displayResults( int maxResultsToDisplay, double elapsedTime ) {
        ...
        int cnt = 0;
        /*while(cnt<50){
            String csvRep = displayableFileName( engine.index.docNames.get( results.get(cnt).docID )) +";";
            csvRep += String.format( "%.5f", results.get(cnt).score);
            System.out.println(csvRep);
            cnt++;
        }*/
        for ( i=0; i<results.size() && i<maxResultsToDisplay; i++ ) {
            ...
            if (displayableFileName( engine.index.docNames.get( results.get(i).docID )).equals("Birding.f")){
                int hf = 1;
            }
            if (displayableFileName( engine.index.docNames.get( results.get(i).docID )).equals("jessicperson.f")){
                int hfs = 1;
                int a = results.get(i).docID;
